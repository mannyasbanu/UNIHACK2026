...
def get_or_compute_signals(ticker: str):
  if ticker not in signals_cache:
    signals_cache[ticker] = generate_signals(ticker)
  return signals_cache[ticker]
...
def get_signals(ticker: str):
  try:
    ...
    signals, headlines = get_or_compute_signals(ticker.upper()) # Generate trading signals and get the scored headlines for the given stock ticker (converted to uppercase)
...
def get_headlines(ticker: str): # Define the get_headlines function that takes a stock ticker as input and returns the scored headlines for that ticker
  try:
    cache_key = f"headlines_{ticker.upper()}"
    if cache_key in cache:
      return cache[cache_key]
    _, headlines = get_or_compute_signals(ticker.upper()) # Generate trading signals and get the scored headlines for the given stock ticker (converted to uppercase)
...
def get_backtest(ticker: str): # Define the get_backtest function that takes a stock ticker as input and returns the backtest results for that ticker
  try:
    cache_key = f"backtest_{ticker.upper()}"
    if cache_key in cache:
      return cache[cache_key]
    
    signals, _ = get_or_compute_signals(ticker.upper()) # Generate trading signals for the given stock ticker (converted to uppercase)
    metrics, backtest_df = run_backtest(ticker.upper(), signals) # Run the backtest using the generated signals and get the performance metrics and backtest DataFrame
    result = {
      "metrics": metrics,
...
def get_summary(ticker: str): # Define the get_summary function that takes a stock ticker as input and returns a summary of the latest signal and backtest performance for that ticker
  try:
    cache_key = f"summary_{ticker.upper()}"
    if cache_key in cache:
      return cache[cache_key]
    signals, headlines = get_or_compute_signals(ticker.upper()) # Generate trading signals and get the scored headlines for the given stock ticker (converted to uppercase)
    metrics, _ = run_backtest(ticker.upper(), signals) # Run the backtest using the generated signals and get the performance metrics
